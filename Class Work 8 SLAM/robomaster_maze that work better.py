#!/usr/bin/env python3
"""
RoboMaster EP / EP Core - 4x5 Maze Explorer v16 FAST (Semantic Map + Dead-End Entry + Confirmed Safety)
============================================
...
# v16 FAST TEST PROFILE
# Faster motion for iteration/testing while retaining heading hold, ToF hard-stop,
# projected-destination safety confirmation, and rollback on genuine failures.
# If the real floor is slippery, reduce MOVE_SPEED_MPS and TURN_MAX_DPS first.
...
# v14: ToF turret is intentionally tilted slightly downward.
# RoboMaster pitch convention: positive = up, negative = down.
# Change this single value if the sensor still aims too high/low.
GIMBAL_PITCH_TARGET_DEG = -4.0
...
# v16: entering a newly discovered cell is different from blindly driving into
# an obstacle. A scan may correctly report an OPEN passage of ~0.70-0.80 m even
# when the destination cell is a dead end, so the far wall can legitimately be
# only ~0.10-0.15 m from the ToF at the destination center.
#
# For NEW/FREE_UNVISITED cells the far wall of a dead end is EXPECTED. v16 no
# longer compares live projected clearance against the initial prediction as a
# hard threshold, because ToF geometry and floating-point noise caused false
# aborts (for example 0.2996 < 0.3000). We only abort if the projected clearance
# at the destination center enters a genuinely unsafe zone and a median recheck
# confirms it.
FRONTIER_ENTRY_ABSOLUTE_MIN_PROJECTED_M = 0.075
FRONTIER_ENTRY_CLEARANCE_DROP_TOL_M = 0.120
FRONTIER_ENTRY_HARD_STOP_M = 0.080
FRONTIER_ENTRY_SLOW_PROJECTED_M = 0.18
FRONTIER_ENTRY_CRAWL_PROJECTED_M = 0.10
FRONTIER_ENTRY_NEAR_TARGET_REMAINING_M = 0.060
FRONTIER_ENTRY_NEAR_TARGET_MIN_FRONT_M = 0.085
# v16: do not rollback on tiny floating-point / ToF deviations such as
# 0.2996 < 0.3000. Only a genuinely unsafe projected destination clearance
# can abort a frontier entry, and it must be confirmed by a short median read.
FRONTIER_ENTRY_CONFIRM_SAMPLES = 3
FRONTIER_ENTRY_CONFIRM_DELAY_S = 0.008
FRONTIER_ENTRY_PROJECTED_ABORT_M = 0.075
...
TRUSTED_RETRACE_SLOW_MPS = 0.18
TRUSTED_RETRACE_CRAWL_MPS = 0.10
...
SCAN_YAW_HOLD_KP = 1.75
SCAN_YAW_HOLD_DEADBAND_DEG = 0.30
SCAN_YAW_HOLD_MIN_DPS = 3.0
SCAN_YAW_HOLD_MAX_DPS = 18.0
...
CENTER_GIMBAL_YAW_KP = 2.0
CENTER_GIMBAL_PITCH_KP = 1.6
CENTER_GIMBAL_MIN_DPS = 8.0
CENTER_GIMBAL_MAX_YAW_DPS = 80.0
CENTER_GIMBAL_MAX_PITCH_DPS = 40.0
CENTER_GIMBAL_YAW_TOL_DEG = 1.2
CENTER_GIMBAL_PITCH_TOL_DEG = 1.2
...
HEADING_LOCK_FINE_MAX_DPS = 28.0
    ...
    *,
    ...
    try:
        while time.monotonic() < deadline:
            ...
            # Yaw still targets the physical zero axis. Pitch intentionally targets
            # GIMBAL_PITCH_TARGET_DEG so the ToF points slightly downward.
            # Use RAW mechanical angles here; do not wrap gimbal yaw.
            yaw_err = -float(st.gimbal_yaw)
            pitch_err = GIMBAL_PITCH_TARGET_DEG - float(st.gimbal_pitch)
    ...
    pitch_err = GIMBAL_PITCH_TARGET_DEG - float(final.gimbal_pitch)
    ...
    if abs(pitch_err) > CENTER_GIMBAL_PITCH_TOL_DEG:
        print(
            f"[GIMBAL WARN] centered yaw OK but pitch={final.gimbal_pitch:+.1f} deg "
            f"target={GIMBAL_PITCH_TARGET_DEG:+.1f}; continuing"
        )

    print(
        f"[CENTER DONE] gimbal_yaw={final.gimbal_yaw:+.2f} "
        f"gimbal_pitch={final.gimbal_pitch:+.2f} "
        f"target_pitch={GIMBAL_PITCH_TARGET_DEG:+.1f}"
    )
...
def calibrate_yaw_sign(chassis) -> None:
    ...
    residual = rotate_to_yaw(chassis, y0, timeout_s=3.0, max_dps=20.0)
...
def scan_360(
    ...
    *,
    ...
    gimbal_moveto(
        gimbal, SCAN_START_DEG, GIMBAL_PITCH_TARGET_DEG, speed_dps=90.0,
    ...
    print(
        f"[SCAN] closed-loop gimbal center -> yaw=0 / "
        f"pitch={GIMBAL_PITCH_TARGET_DEG:+.1f} deg"
    )
...
# v15 semantic cell map. Edge state answers "can I cross here?" while cell
# state answers "have I discovered / visited this logical cell?"
CELL_UNKNOWN = "UNKNOWN"
CELL_FREE_UNVISITED = "FREE_UNVISITED"
...
def drive_one_cell(
    ...
    *,
    ...
    frontier_entry=True means the target logical cell is known FREE from the 360
    scan but has never been visited. This mode deliberately permits the expected
    far wall of a dead-end cell while still requiring the live ToF clearance to
    stay consistent with what was predicted before motion.
    """
    ...
    # v16: expected end clearance is retained for diagnostics only. It is NOT a
    # hard abort threshold anymore; dead-end geometry can legitimately change the
    # apparent ToF range while the robot advances.
    frontier_expected_end = pre - CELL_SIZE_M
    frontier_projected_floor = max(
        FRONTIER_ENTRY_ABSOLUTE_MIN_PROJECTED_M,
        frontier_expected_end - FRONTIER_ENTRY_CLEARANCE_DROP_TOL_M,
    )

    if trusted_retrace:
        mode_text = "TRUSTED_RETRACE"
    elif frontier_entry:
        mode_text = "FRONTIER_ENTRY"
    else:
        mode_text = "NORMAL"
    print(
        ...
        + (
            f" | expected_end={frontier_expected_end:.2f}m"
            if frontier_entry and not trusted_retrace else ""
        )
    )
    ...
    try:
        while True:
            ...
            if trusted_retrace:
                active_hard_stop = TRUSTED_RETRACE_HARD_STOP_M
            elif frontier_entry:
                active_hard_stop = FRONTIER_ENTRY_HARD_STOP_M
            else:
                active_hard_stop = ABSOLUTE_EMERGENCY_STOP_M
            ...
            frontier_near_target_accept = (
                frontier_entry
                and not trusted_retrace
                and remaining <= FRONTIER_ENTRY_NEAR_TARGET_REMAINING_M
                and front >= FRONTIER_ENTRY_NEAR_TARGET_MIN_FRONT_M
                and projected_end_clearance >= FRONTIER_ENTRY_PROJECTED_ABORT_M
                and abs(lateral) <= NEAR_TARGET_ACCEPT_MAX_LATERAL_M
                and abs(yaw_err) <= NEAR_TARGET_ACCEPT_MAX_YAW_ERR_DEG
            )
            if frontier_near_target_accept:
                print(
                    f"[FRONTIER CELL SNAP] new cell reached {progress:.3f}m/"
                    f"{CELL_SIZE_M:.3f}m; remaining={remaining:.3f}m "
                    f"front={front:.3f}m projected={projected_end_clearance:.3f}m "
                    f"safe_limit={FRONTIER_ENTRY_PROJECTED_ABORT_M:.3f}m -> accept FREE cell"
                )
                break
            ...
            if frontier_entry and not trusted_retrace:
                # v16 FIX: do not reject a FREE_UNVISITED cell merely because the
                # live projected clearance is a few mm below the scan-time estimate.
                # A far wall at the end of a dead end is normal. Only stop when
                # continuing to the destination CENTER would leave truly unsafe
                # clearance, and confirm that with a short median ToF re-read.
                if projected_end_clearance < FRONTIER_ENTRY_PROJECTED_ABORT_M:
                    confirm_front = tof_median(
                        samples=FRONTIER_ENTRY_CONFIRM_SAMPLES,
                        delay_s=FRONTIER_ENTRY_CONFIRM_DELAY_S,
                    )
                    if confirm_front is None:
                        reason = "TOF_INVALID_FRONTIER_CONFIRM"
                        break
                    confirm_projected = confirm_front - remaining
                    if confirm_projected < FRONTIER_ENTRY_PROJECTED_ABORT_M:
                        reason = (
                            f"FRONTIER_PROJECTED_UNSAFE_"
                            f"{confirm_projected:.3f}m<"
                            f"{FRONTIER_ENTRY_PROJECTED_ABORT_M:.3f}m"
                        )
                        break
                    print(
                        f"[FRONTIER SAFETY RECHECK OK] first_projected="
                        f"{projected_end_clearance:.3f}m confirm="
                        f"{confirm_projected:.3f}m -> continue"
                    )
            elif (
                not relax_projected
            ...
            if trusted_retrace:
                ...
                # New/unvisited cells are approached conservatively, especially
                # when the map predicts a wall shortly after the cell center.
                speed = MOVE_SPEED_MPS
                if (
                    front < FRONT_SLOWDOWN_M
                    or remaining < 0.22
                    or projected_end_clearance < FRONTIER_ENTRY_SLOW_PROJECTED_M
                ):
                    speed = MOVE_SLOW_MPS
                if (
                    remaining < 0.16
                    or projected_end_clearance < FRONTIER_ENTRY_CRAWL_PROJECTED_M
                ):
...
class MazeExplorer:
    ...
    def __init__(self, output_dir: str):
        ...
        # Cells seen through an OPEN edge but not entered yet. This is the
        # semantic FREE/UNVISITED layer of the map.
        self.discovered_free = {self.current}
    ...
    def update_from_scan(self, scan: Sequence[ScanSample]) -> None:
        ...
        for rel_angle, rel_step in relative_dirs:
            ...
            if status == EDGE_OPEN and in_bounds(nb):
                self.discovered_free.add(nb)
    ...
    def cell_state(self, cell: Tuple[int, int]) -> str:
        ...
        if cell in self.visited:
            return CELL_VISITED
        if cell in self.discovered_free:
            return CELL_FREE_UNVISITED
        return CELL_UNKNOWN

    def _render_edge_h(self, cell: Tuple[int, int], direction: int) -> str:
        nb = neighbor(cell, direction)
        status = EDGE_BOUNDARY if not in_bounds(nb) else self.get_edge(cell, direction)
        if status in (EDGE_WALL, EDGE_BOUNDARY):
            return "---"
        if status == EDGE_OPEN:
            return "   "
        if status == EDGE_OBSTACLE:
            return "XXX"
        return " ? "

    def _render_edge_v(self, cell: Tuple[int, int], direction: int) -> str:
        nb = neighbor(cell, direction)
        status = EDGE_BOUNDARY if not in_bounds(nb) else self.get_edge(cell, direction)
        if status in (EDGE_WALL, EDGE_BOUNDARY):
            return "|"
    ...
    def render_ascii_map(self) -> str:
        """Human-readable map used both on screen and in maze_map.txt.

        Cell symbols: R=current, V=visited, F=free/unvisited, ?=unknown.
        Edge symbols: blank=open, ---/|=wall/boundary, X=blocked/obstacle, ?=unknown.
        """
        lines = [
            "Legend: R=current V=visited F=FREE-unvisited ?=unknown |/---=wall X=blocked",
            f"scan_cycle={self.scan_cycle} visited={len(self.visited)}/{MAZE_ROWS*MAZE_COLS}",
        ]
        for r in range(MAZE_ROWS):
            ...
            for c in range(MAZE_COLS):
                cell = (r, c)
                top.append("+" + self._render_edge_h(cell, 0))
                state = self.cell_state(cell)
                symbol = {
                    CELL_CURRENT: "R",
                    CELL_VISITED: "V",
                    CELL_FREE_UNVISITED: "F",
                    CELL_UNKNOWN: "?",
                }[state]
                mid.append(self._render_edge_v(cell, 3) + f" {symbol} ")
            top.append("+")
            # East boundary/edge of the final cell.
            last = (r, MAZE_COLS - 1)
            mid.append(self._render_edge_v(last, 1))
        ...
        r = MAZE_ROWS - 1
        for c in range(MAZE_COLS):
            bottom.append("+" + self._render_edge_h((r, c), 2))
        bottom.append("+")
        lines.append("".join(bottom))
        return "\n".join(lines)

    def print_map(self) -> None:
        print("[SEMANTIC MAP]\n" + self.render_ascii_map())
    ...
    def save(self) -> None:
        ...
        for r in range(MAZE_ROWS):
            for c in range(MAZE_COLS):
                ...
                cell_rows.append({
                    ...
                    "discovered_free": cell in self.discovered_free,
                })
        ...
        payload = {
            "exploration_algorithm": "Semantic FREE/VISITED cell map + edge occupancy + Frontier-Trémaux exploration",
            ...
            "free_unvisited_cells": sorted([
                list(c) for c in self.discovered_free if c not in self.visited
            ]),
            "cells": cell_rows,
        ...
        with open(
            os.path.join(self.output_dir, "maze_map.txt"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(self.render_ascii_map() + "\n")
    ...
    def run(self, chassis, gimbal, logger: Logger) -> None:
        ...
        print(
            "[EXPLORE] policy = scan 360 -> semantic FREE/VISITED map -> local frontier -> "
        ...
        for action in range(1, MAX_ACTIONS + 1):
            ...
            frontier_entry = nb not in self.visited
            trusted_retrace = (
                not frontier_entry
            ...
            if frontier_entry:
                print(
                    f"[MAP TARGET] {nb} is FREE_UNVISITED -> use frontier-entry "
                    "dead-end-aware safety profile"
                )
            if trusted_retrace:
                print(
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"[GIMBAL] self-test -20 -> +20 -> yaw center / "
            f"pitch={GIMBAL_PITCH_TARGET_DEG:+.1f} deg"
        )
        gimbal_moveto(gimbal, -20.0, GIMBAL_PITCH_TARGET_DEG, speed_dps=80.0, timeout_s=3.0, verify_yaw_tol_deg=STARTUP_SELFTEST_YAW_TOL_DEG)
        gimbal_moveto(gimbal, +20.0, GIMBAL_PITCH_TARGET_DEG, speed_dps=80.0, timeout_s=3.0, verify_yaw_tol_deg=STARTUP_SELFTEST_YAW_TOL_DEG)
        gimbal_moveto(gimbal, 0.0, GIMBAL_PITCH_TARGET_DEG, speed_dps=80.0, timeout_s=3.0, verify_yaw_tol_deg=STARTUP_SELFTEST_YAW_TOL_DEG)
        ...
        print("[SAFETY] v16 FAST + canonical heading lock + semantic map + confirmed frontier safety + rollback watchdog enabled; keep Ctrl+C ready")
